...
enum class ParseResultType : uint8_t {
    LIST, OPTIONAL, CHOICE, EXPRESSION, IDENTIFIER, KEYWORD, STATEMENT, EXTENSION, INVALID
};
...
struct IdentifierParseResult : ParseResult {
    ...
    explicit IdentifierParseResult(string identifier_p) :
       ParseResult(TYPE), identifier(std::move(identifier_p)) {}
};
...
struct KeywordParseResult : ParseResult {
    ...
    explicit KeywordParseResult(string keyword_p) :
       ParseResult(TYPE), keyword(std::move(keyword_p)) {}
};
...
struct OptionalParseResult : ParseResult {
    ...
    explicit OptionalParseResult(ParseResult* result_p) :
       ParseResult(TYPE), optional_result(result_p) {}
};
