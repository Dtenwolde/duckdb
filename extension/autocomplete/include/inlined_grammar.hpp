...
const char INLINED_PEG_GRAMMAR[] = {
	...
	"ExcludeList <- 'EXCLUDE'i (ExcludeNameList / SingleExcludeName)\n"
	"ExcludeNameList <- Parens(List(ExcludeName))\n"
	"SingleExcludeName <- ExcludeName\n"
	...
	"CaseExpression <- 'CASE'i Expression? CaseWhenThen+ CaseElse? 'END'i\n"
	"CaseWhenThen <- 'WHEN'i Expression 'THEN'i Expression\n"
	"CaseElse <- 'ELSE'i Expression\n"
	"TypeLiteral <- ColId StringLiteral\n"
	"IntervalLiteral <- 'INTERVAL'i IntervalParameter Interval?\n"
	...
	"PrefixOperator <- NotPrefixOperator / MinusPrefixOperator / PlusPrefixOperator / TildePrefixOperator\n"
	"NotPrefixOperator <- 'NOT'i\n"
	"MinusPrefixOperator <- '-'i\n"
	"PlusPrefixOperator <- '+'i\n"
	"TildePrefixOperator <- '~'i\n"
	"ListExpression <- 'ARRAY'i? BoundedListExpression\n"
	"BoundedListExpression <- '[' List(Expression)? ']'\n"
	"StructExpression <- '{' List(StructField)? '}'\n"
	"StructField <- ColIdOrString ':'i Expression\n"
	...
	"Parameter <- QuestionMarkParameter / NumberedParameter / ColLabelParameter\n"
	...
	"LikeOrSimilarTo <- LikeExpressionType / IlikeExpressionType / GlobExpressionType / SimilarToExpressionType\n"
	"LikeExpressionType <- 'LIKE'i\n"
	"IlikeExpressionType <- 'ILIKE'i\n"
	"GlobExpressionType <- 'GLOB'i\n"
	"SimilarToExpressionType <- 'SIMILAR'i 'TO'i\n"
	"InOperator <- 'NOT'i? 'IN'i\n"
	"IsOperator <- 'IS'i 'NOT'i? DistinctFrom?\n"
	"DistinctFrom <- 'DISTINCT'i 'FROM'i\n"
	"ConjunctionOperator <- ConjunctionOr / ConjunctionAnd\n"
	"ConjunctionOr <- 'OR'i\n"
	"ConjunctionAnd <- 'AND'i\n"
	"ComparisonOperator <- '=' / '<=' / '>=' / '<' / '>' / '<>' / '!=' / '=='\n"
	...
	"ExtractExpression <- 'EXTRACT'i Parens(ExtractArgument 'FROM'i Expression)\n"
	"LambdaExpression <- 'LAMBDA'i List(ColIdOrString) ':' Expression\n"
	"NullIfExpression <- 'NULLIF'i Parens(Expression ',' Expression)\n"
	"PositionExpression <- 'POSITION'i Parens(SingleExpression 'IN'i SingleExpression)\n"
	...
	"CreateViewStmt <- 'RECURSIVE'i? 'VIEW'i IfNotExists? QualifiedName InsertColumnList? 'AS'i SelectStatementInternal\n"
	...
	"SummarizeRule <- 'SUMMARIZE'i\n"
	"ShowOrDescribe <- ShowRule / DescribeRule / DescRule\n"
	"ShowRule <- 'SHOW'i\n"
	"DescribeRule <- 'DESCRIBE'i\n"
	"DescRule <- 'DESC'i\n"
	...
	"PragmaAssign <- SettingName '=' Expression\n"
	...
	"CreateStatement <- 'CREATE'i OrReplace? Temporary? CreateStatementVariation\n"
	...
	"TempPersistent <- 'TEMP'i\n"
	"TemporaryPersistent <- 'TEMPORARY'i\n"
	"CreateTableStmt <- 'TABLE'i IfNotExists? QualifiedName (CreateTableAs / CreateColumnList) CommitAction?\n"
	"CreateTableAs <- IdentifierList? 'AS'i SelectStatementInternal WithData?\n"
	...
	"QualifiedName <- CatalogReservedSchemaIdentifierOrStringLiteral / SchemaReservedIdentifierOrStringLiteral / TableNameIdentifierOrStringLiteral\n"
	"CatalogReservedSchemaIdentifierOrStringLiteral <- CatalogQualification ReservedSchemaQualification ReservedIdentifierOrStringLiteral\n"
	"SchemaReservedIdentifierOrStringLiteral <- SchemaQualification ReservedIdentifierOrStringLiteral\n"
	"TableNameIdentifierOrStringLiteral <- IdentifierOrStringLiteral\n"
	...
	"UpdateAction <- 'ON'i 'UPDATE'i KeyAction\n"
	"DeleteAction <- 'ON'i 'DELETE'i KeyAction\n"
	...
	"VersionNumber <- 'VERSION'i Identifier\n"
	...
	"MaterializedViewEntry <- 'MATERIALIZED'i 'VIEW'i\n"
	...
	"UpdateTarget <- UpdateTargetNoAlias / UpdateTargetAlias\n"
	"UpdateTargetNoAlias <- BaseTableName 'SET'i\n"
	"UpdateTargetAlias <- BaseTableName UpdateAlias? 'SET'i\n"
	"UpdateAlias <- 'AS'i? ColId\n"
	"UpdateSetClause <- UpdateSetElementList / UpdateSetColumnListExpression\n"
	"UpdateSetElementList <- List(UpdateSetElement)\n"
	"UpdateSetColumnListExpression <- Parens(List(ColumnName)) '=' Expression\n"
	...
	"ByNameOrPosition <- 'BY'i InsertByName / InsertByPosition\n"
	"InsertByName <- 'NAME'i\n"
	"InsertByPosition <- 'POSITION'i\n"
	...
	"TableAlias <- 'AS'i? IdentifierOrStringLiteral ColumnAliases?\n"
	...
	"CrossJoinPrefix <- 'CROSS'i\n"
	"NaturalJoinPrefix <- 'NATURAL'i JoinType?\n"
	"PositionalJoinPrefix <- 'POSITIONAL'i\n"
	"FullJoin <- 'FULL'i 'OUTER'i?\n"
	"LeftJoin <- 'LEFT'i 'OUTER'i?\n"
	"RightJoin <- 'RIGHT'i 'OUTER'i?\n"
	"SemiJoin <- 'SEMI'i\n"
	"AntiJoin <- 'ANTI'i\n"
	"InnerJoin <- 'INNER'i\n"
	...
	"DescendingOrder <- 'DESC'i / 'DESCENDING'i\n"
	"AscendingOrder <- 'ASC'i / 'ASCENDING'i\n"
	"NullsFirstOrLast <- NullsFirst / NullsLast\n"
	"NullsFirst <- 'NULLS'i 'FIRST'i\n"
	"NullsLast <- 'NULLS'i 'LAST'i\n"
	...
	"LimitAll <- 'ALL'i\n"
	"LimitLiteralPercent <- NumberLiteral 'PERCENT'i\n"
	...
	"ReadOrWrite <- 'READ'i ReadOnlyOrReadWrite\n"
	...
	"EnumSelectType <- 'ENUM'i Parens(SelectStatementInternal)\n"
	"EnumStringLiteralList <- 'ENUM'i Parens(List(StringLiteral))\n"
	...
	"CopySelect <- Parens(SelectStatementInternal) 'TO'i CopyFileName CopyOptions?\n"
	...
	"ChangeNullability <- DropOrSet 'NOT'i 'NULL'i\n"
	"DropOrSet <- 'DROP'i / 'SET'i\n"
	...
	"IntervalInterval <- 'INTERVAL'i Interval?\n"
	"IntervalNumber <- 'INTERVAL'i Parens(NumberLiteral)\n"
	...
	"IntType <- 'INT'i\n"
	"IntegerType <- 'INTEGER'i\n"
	"SmallintType <- 'SMALLINT'i\n"
	"BigintType <- 'BIGINT'i\n"
	"RealType <- 'REAL'i\n"
	"BooleanType <- 'BOOLEAN'i\n"
	"DoubleType <- ('DOUBLE'i 'PRECISION'i)\n"
	"FloatType <- 'FLOAT'i Parens(NumberLiteral)?\n"
	"DecimalType <- 'DECIMAL'i TypeModifiers?\n"
	"DecType <- 'DEC'i TypeModifiers?\n"
	"NumericModType <- 'NUMERIC'i TypeModifiers?\n"
	...
	"TableMacroDefinition <- 'TABLE'i SelectStatementInternal\n"
	"CommentStatement <- 'COMMENT'i 'ON'i CommentOnType DottedIdentifier 'IS'i CommentValue\n"
	...
	"CommentTable <- 'TABLE'i\n"
	"CommentSequence <- 'SEQUENCE'i\n"
	"CommentFunction <- 'FUNCTION'i\n"
	"CommentMacroTable <- 'MACRO'i 'TABLE'i\n"
	"CommentMacro <- 'MACRO'i\n"
	"CommentView <- 'VIEW'i\n"
	"CommentDatabase <- 'DATABASE'i\n"
	"CommentIndex <- 'INDEX'i\n"
	"CommentSchema <- 'SCHEMA'i\n"
	"CommentType <- 'TYPE'i\n"
	"CommentColumn <- 'COLUMN'i\n"
