...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformInstallStatement(PEGTransformer &transformer,
	...
	auto &version_opt = list_pr.Child<OptionalParseResult>(4);
	if (version_opt.HasResult()) {
		// TODO(dtenwolde) test this path
		info->version = transformer.Transform<string>(version_opt.optional_result);
	}
