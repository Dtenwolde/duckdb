...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformExecuteStatement(PEGTransformer &transformer, optional_ptr<ParseResult> parse_result) {
	...
	if (table_function_opt.HasResult()) {
		idx_t param_idx = 0;
		auto table_function_arguments = transformer.Transform<vector<unique_ptr<ParsedExpression>>>(table_function_opt.optional_result);
