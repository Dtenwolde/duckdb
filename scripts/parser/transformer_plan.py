...
def plan_extraction(node, rule_types, excluded_rules, identifier_override_rules, optional_semantic_values=False):
...
def emit_extraction(plan, source_expr, target_name=None, indent="\t", declare=True, depth=0):
    ...
    if plan.kind == ExtractionKind.CHOICE:
        ...
        return emit_extraction(plan.child, f"{choice_expr}.GetResult()", target_name, indent, declare, depth)
    ...
    if plan.kind in (ExtractionKind.LIST, ExtractionKind.REPEAT):
        ...
        lines.extend(emit_extraction(plan.child, f"{item_name}.get()", value_name, indent + "\t", depth=depth + 1))
        value_expr = f"std::move({value_name})" if plan.child.by_value else value_name
...
def _classify_recursive(
    ...
    plan = plan_extraction(node, rule_types, excluded_rules, identifier_override_rules, optional_semantic_values)
    if plan is None:
        return None
    if plan.kind == ExtractionKind.SKIP:
